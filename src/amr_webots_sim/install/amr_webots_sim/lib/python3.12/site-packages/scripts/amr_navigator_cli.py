...
import sys
import time
from scripts.amr_waypoint_navigator import AMRWaypointNavigator
...
def main():
    ...
    print(f"Initializing navigation for robot '{args.robot}' with {len(waypoints)} waypoints:")
    ...
    # Create navigator
    navigator = AMRWaypointNavigator(args.robot)
    
    # Configure navigator
    navigator.linear_speed = args.speed
    navigator.angular_speed = args.angular_speed
    navigator.position_tolerance = args.tolerance
    
    # Set waypoints and start navigation
    navigator.set_waypoints(waypoints)
    
    print(f"Robot {args.robot} is now navigating to waypoints.")
    ...
    try:
        ...
        print(f"Robot {args.robot} has completed all waypoints!")
